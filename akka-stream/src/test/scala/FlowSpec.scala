...
final class FlowSpec(implicit ee: ExecutionEnv)
  ...
  "Bulk insert flow" should {
    ...
    def bulkInsertSpec(
      ...
      label >> {
        def spec(coll: BSONCollection, decider: Supervision.Decider)(
          implicit
          w: BSONDocumentWriter[Int]): Future[Int] = {
        ...
        implicit val intWriter = BSONDocumentWriter[Int] { i =>
          if (i % 2 == 0) BSONDocument("number" -> i)
          else throw new Exception("Foo")
        }

        "with 0 successful batch & failure using stop-on-error" in {
          val coll = db[BSONCollection](
            s"bulkinsert1_${System identityHashCode label}")
        ...
        "with 3 successful batches over 5 using resume-on-error" in {
          val coll = db[BSONCollection](
            s"bulkinsert2_${System identityHashCode label}")
        ...
        "with everything successful" in {
          implicit val intWriter = BSONDocumentWriter[Int] { i =>
            BSONDocument("number" -> i)
          }
          val coll = db[BSONCollection](
            s"bulkinsert3_${System identityHashCode label}")
  ...
  "Insert flow" should {
    ...
    def insertSpec(
      ...
      label >> {
        ...
        "with 0 successful batch & failure using stop-on-error" in {
          val coll = db[BSONCollection](
            s"insert1_${System identityHashCode label}")
        ...
        "with 3 successful batches over 5 using resume-on-error" in {
          val coll = db[BSONCollection](
            s"insert2_${System identityHashCode label}")
        ...
        "with everything successful" in {
          ...
          val coll = db[BSONCollection](
            s"insert3_${System identityHashCode label}")
  ...
  "Bulk update flow" should {
    ...
    def updateSpec(label: String)(tb: TestBuilder) = label >> {
      def spec(
        ...
        Source.fromIterator(() => (0 until 5).map { i =>
          val start = i * 10

          Seq.iterate(start, 10)(_ + 1).map { n =>
            s"id${n}" -> n
          }
        }.iterator).via(flow).runWith(Sink.fold(0) { (c, n) => c + n })
      }

      "with no existing document updated and upsert disabled" in {
        val coll = db[BSONCollection](
          s"upbulk1_${System identityHashCode label}")
      ...
      "with all documents upserted successfully" in {
        val coll = db[BSONCollection](
          s"upbulk2_${System identityHashCode label}")
      ...
      "with 40 documents upserted using resume-on-error" in {
        val coll = db[BSONCollection](
          s"upbulk3_${System identityHashCode label}")
      ...
      "with only first batch upserted using resume-on-error" in {
        val coll = db[BSONCollection](
          s"upbulk4_${System identityHashCode label}")
  ...
  "Update flow" should {
    type TestBuilder = FlowBuilder => ((String, Int) => UpElmt) => Flow[(String, Int), Int, NotUsed]
    ...
    def updateSpec(label: String)(tb: TestBuilder) = label >> {
      def spec(
        ...
        Source.fromIterator(() => (0 until 5).iterator.flatMap { i =>
          val start = i * 10

          Seq.iterate(start, 10)(_ + 1).map { n =>
            s"id${n}" -> n
          }
        }).via(flow).runWith(Sink.fold(0) { (c, n) => c + n })
      }

      "with no existing document updated and upsert disabled" in {
        val coll = db[BSONCollection](
          s"update1_${System identityHashCode label}")
      ...
      "with all documents upserted successfully" in {
        val coll = db[BSONCollection](
          s"update2_${System identityHashCode label}")
      ...
      "with 40 documents upserted using resume-on-error" in {
        val coll = db[BSONCollection](
          s"update3_${System identityHashCode label}")
      ...
      "with only first batch upserted using resume-on-error" in {
        val coll = db[BSONCollection](
          s"update4_${System identityHashCode label}")
