...
class CursorSpec(implicit ee: ExecutionEnv)
  ...
  "BSON collection" should {
    ...
    "read empty cursor" >> {
      def cursor(implicit ee: ExecutionEnv): PlayIterateesCursor[BSONDocument] =
    ...
    "stop on error" >> {
      ...
      def scol(n: String = coll2.name)(implicit ee: ExecutionEnv) =
  ...
  "BSON Cursor" should {
    ...
    def toList(implicit ee: ExecutionEnv) =
      Iteratee.fold[Int, List[Int]](List.empty[Int]) { (l, i) => i :: l }

    "read from collection" >> {
      def collection(n: String)(implicit ee: ExecutionEnv) = {
      ...
      def cursor(n: String)(implicit ee: ExecutionEnv): PlayIterateesCursor[Int] = {
    ...
    "read from capped collection" >> {
      def collection(n: String, database: DB)(implicit ee: ExecutionEnv) = {
      ...
      @inline def tailable(n: String, database: DB = db)(implicit ee: ExecutionEnv) = {
        implicit val reader = IdReader
