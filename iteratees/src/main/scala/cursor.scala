...
sealed trait PlayIterateesCursor[T] extends Cursor[T] {
  ...
  def enumerator(
      ...
      err: ErrorHandler[Unit] = FailOnError[Unit]()
    )(implicit
  ...
  def bulkEnumerator(
      ...
      ctx: ExecutionContext
    ): Enumerator[Iterator[T]]
}

final class PlayIterateesCursorImpl[T](val wrappee: Cursor[T])
    extends PlayIterateesCursor[T]
    with WrappedCursor[T] {
  import Cursor.{ Cont, Fail, State }

  private def errorHandler[A](
      chan: Concurrent.Channel[A],
      err: ErrorHandler[Unit]
    ): ErrorHandler[Unit] = {
  ...
  override def enumerator(
    ...
    Concurrent.unicast[T] { chan =>
      wrappee
        .foldWhile({}, maxDocs)(
          (_, res) => Cont(chan push res),
          errorHandler(chan, err)
        )
        .onComplete { case _ => chan.eofAndEnd() }
    }

  override def bulkEnumerator(
      maxDocs: Int = Int.MaxValue,
      err: ErrorHandler[Unit] = FailOnError[Unit]()
    )(implicit
      ctx: ExecutionContext
    ): Enumerator[Iterator[T]] = Concurrent.unicast[Iterator[T]] { chan =>
    wrappee
      .foldBulks({}, maxDocs)(
        (_, bulk) => Cont(chan push bulk),
        errorHandler(chan, err)
      )
      .onComplete { case _ => chan.eofAndEnd() }
  }
...
final class PlayIterateesFlattenedCursor[T](
  ...
  override def enumerator(
    ...
    Enumerator.flatten(cursor.map(_.enumerator(maxDocs, err)))
  ...
  override def bulkEnumerator(
    ...
    Enumerator.flatten(cursor.map(_.bulkEnumerator(maxDocs, err)))
}
