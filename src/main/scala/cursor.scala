...
sealed trait AkkaStreamCursor[T] extends Cursor[T] {
   ...
   * (see [[reactivemongo.api.QueryOpts.batchSizeN]]).
   *
   * @param maxDocs $maxDocsParam
   * @param err $errParam
   * @param m $materializerParam
   */
  def bulkSource(maxDocs: Int = Int.MaxValue, err: ErrorHandler[Option[Iterator[T]]] = FailOnError())(implicit m: Materializer): Source[Iterator[T], NotUsed]
   ...
   * @param m $materializerParam
   */
  def documentSource(maxDocs: Int = Int.MaxValue, err: ErrorHandler[Option[T]] = FailOnError())(implicit m: Materializer): Source[T, NotUsed]
