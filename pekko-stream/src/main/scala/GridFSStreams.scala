 ...
 */
sealed trait GridFSStreams extends GridFSCompat {
  private[pekkostream] type Pack <: SerializationPack
  ...
  final def sinkWithMD5[Id <: pack.Value](
      ...
      ec: ExecutionContext
    ): Sink[ByteString, Future[ReadFile[Id]]] = {
    import java.security.MessageDigest

    sink[Id, MessageDigest](
      file,
   ...
   */
  final def sink[Id <: pack.Value, M](
      file: FileToSave[Id],
      digestInit: => M,
    ...
    Sink
      .foldAsync[StoreState[Id, M], Array[Byte]](initial) { (prev, chunk) =>
  ...
  private final class StoreState[Id <: pack.Value, M](
    ...
    def feed(
      ...
      Future.traverse(zipped) { ci => writeChunk(n + ci._2, ci._1) }.map { _ =>
...
object GridFSStreams {
  ...
  private[pekkostream] lazy val logger =
    reactivemongo.util.LazyLogger("reactivemongo.pekkostream.GridFSStreams")

  /** Returns an Pekko-stream support for given GridFS. */
